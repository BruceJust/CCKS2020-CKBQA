...
def process_one_entity( question, mentions):
    ans = process_one_entity_forward(question, mentions)
    if ans == '':
        ans = process_one_entity_back(question, mentions)
    return ans

def process_one_entity_forward( question, mentions):
    start = time.time()
    ...
    scores = score_fun(question, pairs, back=False)
    max_score = 0
    best_pattern = []
    for item in scores:
        if item[0] > max_score:
    ...
    # 如果有多个候选relation一样的分数，优先选relation少的
        ...
        for item in best_pattern:
            if len(item[3]) == min_len:
                ans = GetAnsFword(item[2][0], item[3])
                break
    return ans

def process_one_entity_back( question, mentions):
    start = time.time()
    ...
    # 用BERT计算相似度
    scores = score_fun(question, pairs, back=True)
    max_score = 0
    best_pattern = []
    for item in scores:
        if item[0] > max_score:
...
def process_two_entity( question, mentions):
    mention_pairs = []
    for i in mentions[0]:
        for j in mentions[1]:
    ...
    # 用BERT计算相似度
    scores_two = score_fun(question, pairs, back=False)
    max_score = 0
    best_pattern = []
    ...
    if len(best_pattern) == 0:
        ...
        ans = GetEntity2Tuple(best_pattern[0][2][0][0], best_pattern[0][3][0], best_pattern[0][3][1], best_pattern[0][2][0][1])
    return ans

def score_fun( question, pairs, back=False):
    dataset = get_sim_example_features_eval(question, pairs, scoring.tokenizer, 50, back)
    start = time.time()
    # 相似度取两个model平均
    # res1 = []
    ...
    res2 = []
    for i in range(len(dataset)):
        res = scoring.sim_model_.predict(dataset[i])
        res2.append(res[0][1])
    scores = []
    for i in range(len(dataset)):
